...
# matrix
#
# # Видно, что получился список списков, где каждый вложенный список представляется собой строку. Тогда чтобы обратиться к отдельному элементу матрицы нужно использовать двойную индексацию
#
# # In[14]:
#
#
...
#
# # ---
# # Если бы в задаче не просили найти индексы, то можно было бы обойтись одним циклом. В этом вся прелесть языка программирования Python
#
# # In[9]:
#
#
...
#
# # ---
# # Со столбцами такой фокус не пройдет, потому что слотбцы не представлены отдельными списками. Поэтому для работы со столбцами применим только первый способ.
...
# # ## Continue
# #
# # Если прошлый раз был рассмотрено ключевое слово `break`, которое дострочно прерывает выполнение цикла, то сейчас рассмотрим ключевое слово `continue`. Если в теле цикла встречается ключевое слово `continue`, то цикл пропускает весь код до конца тела цикла и переходит на следующий шаг.
#
# # Рассмотрим следующую задачу:
# # ```
# # В клетке находятся фазаны и кролики. Известно, что у них 35 голов и 94 ноги. Узнайте число фазанов и число кроликов.
# # ```
# # Количество фазанов и кроликов выражается целым числом, поэтому будем перебирать все возможные комбинации количества кроликов и фазанов, и если их суммарное количество ног равно, указанному в условии, то мы нашли одно из решений.
#
# # In[17]:
#
#
